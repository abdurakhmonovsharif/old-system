...
const sendMessage = async () => {
    ...
    try {
        ...
        const debtsreport = await Promise.all(saleConnectors.map(async (sale) => {
            ...
            const products = await SaleProduct.find({_id: {$in: sale.products}});
            ...
            const debtComment = debts.length > 0 ? debts[debts.length - 1].comment : "";
            const debtId = debts.length > 0 ? debts[debts.length - 1]._id : "";
            const payEndDate = debts.length > 0 ? debts[debts.length - 1].pay_end_date : "";
